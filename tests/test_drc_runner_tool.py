...
def test_run_drc_check_with_params():
    """Test run_drc_check with provided parameters"""
    import pytest
    try:
        result = run_drc_check(quiet=True, lib="TestLib", cell="TestCell", view="layout", tech_node="T28")
        assert isinstance(result, bool), "run_drc_check should return a boolean"
    except Exception as e:
        pytest.skip(f"DRC test requires Virtuoso: {e}")
